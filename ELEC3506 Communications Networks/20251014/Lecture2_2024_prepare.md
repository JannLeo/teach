...
* 选一载波 $c(t)=\cos(2\pi f_ct)$，按比特驱动**幅度/频率/相位**（或它们组合）**离散取值** → 形成\*\*符号（symbol）\*\*序列。
* 每个符号携带 $b=\log_2 M$ 个比特（$M$ 为可区分的取值/星座点个数）；**比特率** $R_b=bR_s$（$R_s$=符号率）。

**频谱占用**（升余弦成形、滚降 $\alpha$）：约 $B \approx \frac{1+\alpha}{2}R_s$。
**容量上限**（香农）：$R_b \le B\log_2(1+\text{SNR})$。

---

## 2) 五种常见调制（一句话 + 适用场景）

### ASK（Amplitude Shift Keying，幅移）

* **变幅度**，相位/频率不变；2-ASK≈开关键。
* **优**：简单；**缺**：对幅度噪声/增益起伏敏感。多用于低速或基带的 PAM 近亲。

### FSK（Frequency Shift Keying，频移）

* **变频率**：不同频点代表不同符号（2/4-FSK…）。
* **优**：可非相干检测、对幅度噪声鲁棒；**缺**：频谱效率低、占带宽宽。常见于低功耗/遥测。

### PSK（Phase Shift Keying，相移）

* **变相位**：BPSK、QPSK、8PSK…
* **优**：BPSK/QPSK 的 $E_b/N_0$ 性能好；**缺**：高阶 PSK 点间距小，对 SNR 要求迅速升高。

### QPSK（Quadrature PSK）

* **PSK 的 M=4 特例**：每符号 2 bit（00、01、11、10），I/Q 正交两路实现。
* **无线主力**：很多系统在低中 SNR 时用 QPSK 起步。

### QAM（Quadrature Amplitude Modulation，正交振幅相移）

* **幅度 + 相位**联合编码（16/64/256-QAM…），星座为二维网格。
* **优**：在给定带宽下**频谱效率最高**；**代价**：需更高 **SNR**、更好的线性度与相位噪声控制。Wi-Fi、蜂窝、线缆调制解调器主力。

> 记忆：**ASK 简、FSK 稳、PSK 抗噪好、QAM 最能“装比特”但最挑 SNR/线性**。

---

## 3) 频谱效率与SNR的权衡（这页红字的内涵）

* **频谱效率** $\eta=\dfrac{R_b}{B}$（bit/s/Hz）。提 $\eta$ 的两条路：

1. 提高 $M$（每符号携带的比特 $b=\log_2M$ 增大）；
2. 提高 $R_s$（但 $R_s$ 受带宽 $B$ 与成形 $\alpha$ 限制）。
* **代价**：$M$ 越大，星座点越密，**最小欧氏距离变小** → 要求更高 SNR（或更强 FEC）。
* **上限**：不论怎么调制，都不能超过香农容量 $C=B\log_2(1+\text{SNR})$。

---

## 4) 两个快算例（上课就用）

1. **16-QAM**，$R_s=1\ \text{Mbaud}$ → $b=\log_2 16=4$，$R_b=4\ \text{Mb/s}$。
2. **滚降 $\alpha=0.25$**、$R_s=5\ \text{Mbaud}$ → 占带 $B\approx\frac{1.25}{2}\times5=3.125\ \text{MHz}$。

---

## 5) 实战搭配（一句话案例）

* **Wi-Fi/4G/5G**：OFDM 子载波上自适应选 BPSK/QPSK/16/64/256-QAM + FEC（LDPC/Polar）。
* **有线电视/DOCSIS**：64/256-QAM；
* **卫星/微波**：QPSK、8PSK、16/32-APSK（星座更适合非线性功放）。

---

## 6) 课堂小结（金句）

> **数字→模拟调制**就是把比特“装”进载波的**幅度/频率/相位**里。
> 频谱是稀缺资源：**想要更高速，就要么更宽带宽、要么更高阶调制（但要更高 SNR/更好线性），并用 FEC 逼近香农极限。**
...
$$
s_0(t)=0,\quad s_1(t)=A\cos(2\pi f_ct)\quad(\text{OOK，开关键})
$$

或者

$$
s_0(t)=A_0\cos(2\pi f_ct),\ s_1(t)=A_1\cos(2\pi f_ct),\ A_1\ne A_0
$$
* **M-ASK（多电平幅移）**：用多个幅度等级（如 4-ASK）承载更多比特/码元（$\log_2M$ bit/符号），但对噪声更敏感。
* **图里的含义**：每个“1 baud”时间内要么出现“高幅度的载波包”，要么保持低/零幅度（对应 0）。

> 记忆：**ASK 只改“高矮”，不改“快慢或起点”（频率/相位不变）**。

---

## 2) 速率、带宽与成形

* 若每符号携带 $b$ 比特（2-ASK 时 $b=1$），则

$$
R_b=b\,R_s
$$
* 采用升余弦成形（滚降 $\alpha$）时，占用带宽约

$$
B\approx \frac{1+\alpha}{2}\,R_s
$$
* 与基带一样，**想要更高比特率**：要么提 $R_s$（吃更多带宽），要么提 $M$（更高阶 ASK，但更挑 SNR）。

---

## 3) 噪声与性能（为什么“Highly susceptible to noise”）

* **幅度直接承载信息** → 对**加性噪声、增益起伏、衰落**非常敏感。
* **OOK（0 与 A）**是**非反极性**的两点星座，相比 **BPSK（±A）**，在相同误比特率下约需**更高 3 dB 的 $E_b/N_0$**（直观：两点之间“距离”更短）。
* **抗噪改进**：相干检测、自动增益控制（AGC）、功放线性度控制、用 FEC 以及避免深衰落的分集/交织等。
...
* **OOK**：红外遥控、低速 RFID/车库门遥控器、超低功耗短距无线。
* **光通信的强度调制**（IM/DD）常可视作“类 ASK”。
* **较少用于高速无线/有线主链路**：现代系统更偏向 **PSK/QAM**（频谱效率更高、抗噪性更好）。
* **类比**：模拟广播里的 **AM** 是“模拟幅度调制”，ASK 是它的数字版本（幅度只取离散值）。

---

## 5) 快速板书例题（课堂 2 分钟）

1. **OOK，$R_s=1\ \text{Mbaud}$、$\alpha=0.25$**：
$R_b=1\ \text{Mb/s}$，占带 $B\approx \frac{1.25}{2}\times1=0.625\ \text{MHz}$。
2. **4-ASK（每符号 2 bit）**，要实现 $R_b=4\ \text{Mb/s}$，最小 $R_s$ 约多少？
$R_s=R_b/2=2\ \text{Mbaud}$，若 $\alpha=0.25$，$B\approx 1.25/2\times2=1.25\ \text{MHz}$。但需更高 SNR 才能把四个幅度清楚地区分开。

---

### 一句话收束

> **ASK**：把比特装进“**幅度**”。它实现简单，但最怕噪声和增益漂移；在带宽受限且信道条件一般时，现实系统更常选 **PSK/QAM** 或配合 FEC 自适应切换。
...
* 载波：$c(t)=\cos(2\pi f_c t)$（频率与相位固定）。
* **映射（典型约定）**

$$
\begin{cases}
\text{bit }1:\ s_1(t)=A\cos(2\pi f_ct) \\
\text{bit }0:\ s_0(t)=0
\end{cases}
$$

也可用两种不同非零幅度，但 OOK 最常见。
* 每个比特周期内是否“有振荡包络”就是判决依据。

---

## 2) 速率、带宽与频谱

* 每符号承载比特数 $b=\log_2 M=\log_2 2=1$，所以

$$
R_b=b\,R_s=R_s
$$
* 采用升余弦成形（滚降 $\alpha$）时，占用带宽近似

$$
B\approx \frac{1+\alpha}{2}\,R_s=\frac{1+\alpha}{2}\,R_b
$$

（与 2-PSK/2-PAM 同量级；$\alpha$ 越小频谱越紧，但实现更难。）

---

## 3) 接收与误码（为什么常说它“怕噪声”）

* **非相干包络检波**：整流+低通取包络，门限判决；实现简单，性能一般。
* **相干检测**：本振相干乘法相关后积分判决，性能最佳但需载波同步。
* **性能对比（AWGN，目标 BER）**

* OOK 与 **BPSK** 相比，在相同 BER 下大约需要 **更高 ≈3 dB 的 $E_b/N_0$**（直觉：两点星座 $\{0,A\}$ 的“距离”比 $\{-A,+A\}$ 小）。
* **信道敏感性**：幅度直接承载信息 → 对**衰落、阴影、AGC 误差**较敏感；因此在多径/衰落信道，BASK 的鲁棒性不如 PSK/QAM（配合相干检测/FEC 的情况下）。
...
* 发送/接收结构非常简单（特别是非相干检波），功耗低。
* 在光通信的 **IM/DD**（强度调制/直接探测）中可以看作“类 OOK”。

**局限**

* 抗噪与抗衰落能力一般，**更依赖 SNR 或 FEC**。
* 频谱效率只能靠提升 $R_s$（吃带宽），无法像 QAM 那样提升每符号比特数。

**典型应用**

* 红外遥控、低速短距无线、简易 RFID、光互联的 OOK 方案等；主流宽带无线/有线主链路更偏向 QPSK/QAM。

---

## 5) 课堂口算小练习

1. BASK，$\alpha=0.25$，若 $R_b=4\,\text{Mb/s}$，所需带宽？

$$
B\approx \frac{1.25}{2}\times 4=2.5\ \text{MHz}
$$
2. 若可用带宽 $B=1.5\,\text{MHz}$、$\alpha=0.25$，最大近似比特率？

$$
R_b\approx \frac{2B}{1+\alpha}=\frac{3}{1.25}\approx \mathbf{2.4\ Mb/s}
$$
...
> **BASK/OOK** = “有载波就是 1，没载波就是 0”。它胜在简单与低成本，但更怕噪声与衰落；要跑得更快或更稳，通常需要更高阶的 **PSK/QAM** 与**FEC**。
...
* `1` → 载波频率较**高**的那段正弦（密集）；
* `0` → 载波频率较**低**的那段正弦（稀疏）；
* 幅度、初始相位不随比特改变（理想 FSK 是**常包络**信号）。

数学上可写作（比特时长 $T_b$）：

$$
s_i(t) = A \cos\!\big(2\pi f_i t\big),\quad i\in\{0,1\},\ t\in[0,T_b)
$$
...
* **每符号 1 bit** ⇒ $\;R_b=R_s$（bit/s = baud）。
* 频谱占用与**频移量** $\Delta f=|f_1-f_0|/2$ 与码率有关。一个常用近似：

$$
\boxed{B \approx 2\big(\Delta f + R_b\big)}
$$

（直观版“卡森公式”：频移越大、码率越高，占带越宽。）

* **特殊形态**：

* **MSK（Minimum-Shift Keying）**：令 $\Delta f=\frac{R_b}{2}$（调制指数 $h=0.5$），频谱最紧凑、包络恒定。
* **GFSK**：先用高斯滤波再 FSK（Bluetooth/BLE 采用），进一步压缩旁瓣。

---

## 3) 检波方式与抗噪性能

* **相干 BFSK（coherent）**：有载波同步，最佳判决性能。正交选频时其误比特率与 **BPSK** 等效（$P_b=Q(\sqrt{2E_b/N_0})$）。
* **非相干 BFSK（non-coherent）**：不对齐相位，结构简单但性能差约 **3 dB**（$P_b\approx \tfrac12 e^{-E_b/(2N_0)}$）。
* **常包络优点**：对**幅度起伏/功放非线性**不敏感，可用高效率非线性功放（适合低功耗、远距离）。
...
| 变化维度          | 幅度     | **频率**        | 相位         |
| 抗幅度噪声         | 弱      | **强**         | 中          |
| 频谱效率（同 $R_b$） | 中      | **较低**（要分开频点） | 中/高（高阶）    |
| 放大器容忍度        | 线性度要求高 | **常包络，容忍度好**  | 相干时要求相位噪声低 |

---

## 5) 应用举例

* **低功耗/窄带**：遥测、无线抄表、部分 RFID、LoRa 族的 CSS（扩频思想与 FSK相关）。
* **蓝牙（经典/BLE）**：采用 **GFSK**（高斯成形的 FSK）。
* **卫星/应急通信**：在强非线性功放场景中常用 MSK/GMSK 类。

---

## 6) 快速练习（课堂 2 分钟）

1. 设 $R_b=10\,\text{kb/s}$，$\Delta f=5\,\text{kHz}$。估算占带？
$\;B\approx 2(\Delta f + R_b)=2(5+10)=\mathbf{30\,kHz}$。
2. 若要求占带 $\le 20\,\text{kHz}$ 且 $R_b=10\,\text{kb/s}$，$\Delta f$ 取多少合适？
解：$2(\Delta f+10)\le 20 \Rightarrow \Delta f\le 0$ ⇒ 说明必须**降低码率**或改用 **MSK/GFSK** 缩旁瓣；FSK 在高 $R_b$ 下带宽压力大，这也是宽带系统更偏向 **QAM/PSK** 的原因。

---

### 一句话收束

> **FSK** 用“**不同频率**”装比特：**抗幅度噪声、功放友好**，但**频谱效率偏低**。低功耗/窄带/远距很常见（GFSK/MSK），而宽带高速则更多使用 **QPSK/QAM**。
...
* 载波：$c(t)=\cos(2\pi f_ct)$
* **BPSK（Binary PSK）** 的两种相位：

$$
s_0(t)=A\cos(2\pi f_ct),\qquad
s_1(t)=A\cos(2\pi f_ct+\pi)=-A\cos(2\pi f_ct)
$$

也可写成差分形式（见下）。

> 直觉：**相位=起点**，BPSK 就是“正相/反相”两种取值（±A），幅度不变、频率不变。

---

## 2) 检测方式

* **相干检测**（需载波同步）：与本振同频同相相关积分后判决，性能最佳。
* **差分检测（DPSK）**：比较相邻符号的相位差（如 DBPSK）；避免显式载波恢复，但误码性能比相干差约 **3 dB**。
...
——在所有二元调制里（相干）**最优**，比 OOK/2-ASK 抗噪 **≈3 dB** 更好。
* **恒包络**（幅度不变）→ 对增益起伏/衰落比 ASK 更稳；但功放需保证相位噪声与频偏受控。

---

## 4) 扩展到高阶 PSK 与 QAM

* **QPSK（4-PSK）**：四个相位（0°, 90°, 180°, 270°），**每符号 2 bit**；
通过 I/Q 正交分路实现，频谱效率翻倍而符号率不变。
* **M-PSK（8PSK 等）**：每符号 $\log_2M$ bit，但**点间夹角更小**，对 SNR 的要求迅速提高。
* 工程上更常用 **QAM**（幅度+相位），在相同 SNR 下可进一步提高频谱效率。

---

## 5) 速率与带宽

* 每符号承载 $b$ bit：$R_b=b\,R_s$。
* 升余弦成形、滚降 $\alpha$ 时，占带近似

$$
B\approx \frac{1+\alpha}{2}\,R_s
$$

对 BPSK：$b=1\Rightarrow R_b=R_s$；对 QPSK：$b=2\Rightarrow R_b=2R_s$（**同样带宽下比特率×2**）。
...
* **ASK**：改幅度，简单但怕噪声/增益漂移；
* **FSK**：改频率，常包络、功放友好，但**占带较宽**；
* **PSK**：改相位，**抗噪最优（BPSK/QPSK）**，频谱效率中高，是现代无线/有线系统核心调制之一。

---

## 7) 快速练习（课堂口算）

1. **QPSK**，符号率 $R_s=1\,\text{Mbaud}$，求比特率。
**解**：$b=2\Rightarrow R_b=2\,\text{Mb/s}$。
2. 升余弦 $\alpha=0.25$，若 BPSK 符号率 $R_s=4\,\text{Mbaud}$，占带约？
**解**：$B\approx \frac{1.25}{2}\times4=2.5\,\text{MHz}$。
3. 目标 BER $10^{-5}$ 时，相干 **BPSK** 与 **OOK** 的 $E_b/N_0$ 相差大约？
**答**：BPSK **约好 3 dB**。
...
> **PSK** 用“相位”装比特：**幅度、频率固定，靠相位跳变**。BPSK/QPSK 既抗噪又省带，是 Wi-Fi、蜂窝、卫星等系统的“基础调制”，高阶场景再叠加 QAM 以提高频谱效率。
...
* 每个符号打包两位，例如高位给 **I（In-phase，余弦）**，低位给 **Q（Quadrature，正弦）**。
2. **“2 Separate PSK modulators”**

* I、Q 各自做 BPSK：把 0/1 映射为 $\pm1$。
3. **“In-phase and Quadrature (out-of-phase)”**

* 两路载波相差 **90°**：$\cos(2\pi f_ct)$ 与 $\sin(2\pi f_ct)$ 正交。
4. **合成波**

* 发射信号：$s(t)=I\cdot \cos(2\pi f_ct)+Q\cdot \sin(2\pi f_ct)$
* 因为 $I,Q\in\{-1,+1\}$，合成后只有 **四个相位**：$45^\circ,\ -45^\circ,\ 135^\circ,\ -135^\circ$（图中用四色/四段示意）。
5. **星座图（左下十字）**

* 横轴为 I，纵轴为 Q。四个点分别对应 **00、01、11、10**（Gray 映射，相邻点只差 1 bit，降低误码）。
...
* 每符号比特数 $b=\log_2 4=2$
* **比特率**：$\boxed{R_b=2R_s}$（同 BPSK 的 $R_b=R_s$ 比，翻倍）
* 升余弦成形滚降 $\alpha$ 时 **占带**：$\boxed{B\approx \frac{1+\alpha}{2}R_s}$
⇒ **同样带宽** $B$ 下，QPSK 的 **比特率**是 BPSK 的 **2 倍**。

---

## 三、误码与同步（怎么“收”）

* 相干检测（载波/相位同步，如 Costas 环）后，分别对 I、Q 判正负即可还原两位。
* AWGN + 相干检测时，**QPSK 的 BER 与 BPSK 等效**（每维像一个 BPSK）：

$$
P_b\approx Q\!\left(\sqrt{\frac{2E_b}{N_0}}\right)
$$
* **Gray 映射**可把符号误判转化为单比特差错，降低 BER。

---

## 四、工程要点与常见变体

* **峰均比/穿越原点**：理想 QPSK 幅度近似恒定；为兼容非线性功放，常用

* **OQPSK**（I、Q 半符号错位，减少 180° 跳变）、
* **$\pi/4$-QPSK**（星座旋转，避免穿越原点）。
* **应用**：Wi-Fi、LTE/5G、卫星链路等在中低 SNR 模式广泛使用 QPSK；SNR 高时切到 16/64/256-QAM 进一步提速。

---

## 五、讲台小练习（1～2 分钟）

1. 已知 $R_s=2\text{ Mbaud},\ \alpha=0.25$。求 $R_b$ 与占带 $B$。

* $R_b=2R_s=4\text{ Mb/s}$；$B\approx\frac{1.25}{2}\times2=1.25\text{ MHz}$。
2. 在星座图上顺时针标 00→01→11→10，这是什么映射？

* **Gray 映射**（任意相邻点仅 1 bit 不同）。

---

### 一句话收束

> **QPSK = 两路正交 BPSK 并行**：同带宽、符号率下把比特率翻倍，且误码性能接近 BPSK，是现代无线/有线系统的“基础工作马”。
