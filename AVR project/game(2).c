...
void move_player(int8_t delta_row, int8_t delta_col)
{
	...
	// Random messages
	srand();
	char messages = {"hit the wall", "wall ahead", "wall in front"};
	// Relocated where the player is
	uint8_t new_player_row = (player_row + delta_row + MATRIX_NUM_ROWS) % MATRIX_NUM_ROWS;
	uint8_t new_player_col = (player_col + delta_col + MATRIX_NUM_COLUMNS) % MATRIX_NUM_COLUMNS;
	
	//Check is WALL ahead
	if (board[new_player_row][new_player_col] & WALL){
		printf("%s/n", messages[rand()%3]);
		return
	}
	// check the new position is the sign of the box on the board
	if (board[new_player_row][new_player_col] & BOX)
	{
		uint8_t new_box_row = (new_player_row+ delta_row + MATRIX_NUM_ROWS) % MATRIX_NUM_ROWS;
		uint8_t new_box_col = (new_box_col + delta_col + MATRIX_NUM_COLUMNS) % MATRIX_NUM_COLUMNS;
		
		// check the vality of the box in the new position
		if(!(board[new_box_row][new_box_col] & (WALL | BOX)))
		{
			// Moving box
			board[new_player_row][new_player_col] ^= BOX; //Remove the box from the place where it is
			board[new_player_row][new_player_col] |= BOX; //Add the box to the new position
				
			// Player's position with boxes
			player_row = new_player_row;
			player_col = new_player_col;
		} else if (board[new_box_row][new_box_col] == WALL) {
			printf("Cannot push box onto wall.");
			return;
		} else if (board[new_box_row][new_box_col] == BOX) {
			printf("Cannot stack boxes."); 
			return;
		} else {
			//No boxes, player moving by himself
			player_row = new_player_row;
			player_col = new_player_col;
		}

	// flash the player again, let him in the new location
	flash_player();
