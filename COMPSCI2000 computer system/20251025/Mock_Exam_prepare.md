...
3. 下面这题是在考“HACK CPU”（Nand2Tetris 课程里的那台机）里**控制逻辑**，图上有很多用 © 标出的“缺失的门/连线”。大号 **X** 标的是其中一段“缺失的控制逻辑”，位于 **A 寄存器输入一侧**。题目要你结合 HACK 指令格式和 ALU 设计，说明这块缺失逻辑到底在做什么。
...
>
> ### 1. 没有进程/线程调度
> - Hack 平台只有**单核、单线程**的硬件模型，CPU 永远运行一条指令流。
