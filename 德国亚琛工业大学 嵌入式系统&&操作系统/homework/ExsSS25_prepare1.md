...
> Profibus 采用总线拓扑；介质访问通过主从机制实现，若有多个主站则用令牌总线管理主站的发送时机。
...
这页有两道题：上半是 **I²C 多主仲裁与时序绘制**，下半是 **Profibus 的拓扑与介质访问控制**。我把题意、做题步骤与结论都说清楚。

---

## 题 1：I²C 两个主机同时发送，谁赢仲裁？怎么画波形？

**题意**

* A 的地址 `0x0a`（二进制 7 位：`0001010`），要给 **B** 发消息，所以在地址阶段会发 **`0x0b` = `0001011`**。
* B 的地址 `0x0b`（`0001011`），要给 **A** 发消息，所以会发 **`0x0a` = `0001010`**。
* 两个主机**完全同步**，从同一时刻开始发 **Start**，SCL 没相位差。
* 让你画 **SCL、SDA\_A、SDA\_B、SDA（总线）** 到“第一次传输完成”为止，并**标出决定胜负的比特**。

**原理**

* I²C 线是开漏/开集 + 上拉，逻辑为 **Wired-AND（线与）**：**0 优先**。
* **仲裁**：每个主机一边发 SDA 一边读 SDA。若自己想送 1，却读到 0，就**输**，马上**停止驱动**（退总线，只监听）。

**怎么画（步骤）**

1. 画若干周期 **SCL** 方波（统一时钟）。
2. 写出两边要发送的**目标地址位流（MSB→LSB）**：

* A→B：`0001011`（A 的 **SDA\_A**）
* B→A：`0001010`（B 的 **SDA\_B**）
3. 在每个比特的采样区间（SCL 高电平稳定期）画出各自的 SDA 输出。
4. 画总线 **SDA**：对同一时刻的 SDA\_A 与 SDA\_B 取“**线与**”（谁拉低就是 0）。
5. **找第一个不相同的比特**：前 6 位相同 `000101`，**第 7 位**不同——A 送 **1**，B 送 **0**，总线变 **0**。

* A 读到“自己送 1 却看到 0”→ **A 输**；
* **B 赢仲裁**，继续完成本次传输（后续 R/W 位、ACK、数据…）。
6. 之后把 **SDA\_A** 画成“释放总线”（不再改变），**SDA** 跟随 B 直到该次传输的 **Stop** 为止。
7. **把“决定胜负的那一位”高亮标注**（最后一位地址位）。

**一句结论**

> **B 赢仲裁**（因为在决定性比特送 0），A 停止发送，等待 B 完成后再重试。

---

## 题 2：Profibus 的拓扑与介质访问如何控制？

**答题要点**

* **拓扑**：**总线拓扑（Bus Topology）**（常见物理层 RS-485 双绞线）。
* **介质访问**：

* 基本是 **主从（Master/Slave）** 轮询从站；
* **多主**通过 **令牌总线（Token Bus）** 管理：只有拿到令牌的主站才能发，令牌在主站间有序传递，避免冲突。

**一句作答模板**

> Profibus 采用总线拓扑；介质访问由主从机制实现，多主通过令牌总线（Token）轮转发送权限。

---

## \[口语化表达]

这页讲两件事。第一，I²C 两个主机同时发怎么不打架：把 A→B 的地址 `0001011` 和 B→A 的地址 `0001010` 对比着发，SDA 是“线与”，谁拉低听谁的，所以最后一位上 **B 送 0、A 送 1**，总线成 0，**B 赢、A 让**；画图时把 SCL、两条各自的 SDA，再合成一条总线 SDA，并把“最后那一位”圈出来即可。第二，Profibus 的网络形状是**总线**，平时**主从轮询**；要是有好几个主站，就用**令牌**轮着说话，谁拿到令牌谁发，天然不冲突。
