...
![image-20250411171318602](./READEME7.assets/image-20250411171318602.png)

这张图解释了 **Kruskal 算法** 中的 **分区性质（Partition Property）**，它是用于最小生成树 (MST) 的一个重要性质。该性质表示：

### 分区性质

1. **定义**：设定图 **G** 的顶点集合被划分为两个子集 **U** 和 **V**。考虑一个从 **U** 到 **V** 的边 **e**，并且该边在这两个子集的连接中具有最小的权重。
2. **结论**：如果 **e** 是连接 **U** 和 **V** 的最小边（即它的权重是所有连接这两个子集的边中最小的），那么 **e** 一定是某个 **最小生成树**（MST）的一部分。

### 解释：

- 在图 **G** 中，边 **e** 的作用是连接两个不同的顶点集 **U** 和 **V**，并且它是这两个集合之间权重最小的边。这个边 **e** 的加入会帮助我们找到最小生成树的一部分。
- **分区性质** 告诉我们，对于任意一个顶点集合的划分，只要该边是连接两个子集之间的最小边，它一定是最小生成树中的一条边。

### 例子：

- 在图中，左侧的子集 **U** 包含顶点 **A, B, E**，右侧的子集 **V** 包含顶点 **C, D, F**。
- 连接 **U** 和 **V** 的边 **e** 的权重是 **2**，是这两个子集之间所有边中最小的。因此，根据 **分区性质**，这条边 **e** 必定会出现在某个最小生成树中。

这个性质在 **Kruskal 算法** 中非常有用，因为它帮助我们从最小权重的边开始构建最小生成树。

![image-20250411171535040](./READEME7.assets/image-20250411171535040.png)

这张图展示了 **Prim 算法** 的实现步骤。Prim 算法是用于找到 **最小生成树**（Minimum Spanning Tree, MST）的一种贪心算法。以下是对图中内容的详细解释：

### 1. **算法概述**:

Prim 算法通过逐步扩展一个已知的生成树，选取每次连接生成树的最小边，直到生成树包含所有的顶点。不同于 Kruskal 算法，它是从某个起点开始生成树，而 Kruskal 是从图中的边开始。
...
- **第5行**：初始化所有顶点的最短路径 **d[v]** 为正无穷（表示初始时无法到达任何其他顶点），并且 **p[v]** 为 NULL（表示没有父节点）。
- **第6行**：起始顶点 **s** 的最短路径 **d[s]** 设置为 0，表示起始顶点与自己之间的路径权重是0。
- **第10行**：构建一个 **优先队列**（PriorityQueue **Q**），该队列按每个顶点的最短路径距离 **d[v]** 排序。最小距离的顶点会被优先选择。
- **第13行**：从队列中取出距离最小的顶点 **m**，将其添加到最小生成树 **T** 中。
- **第14行 - 第20行**：对于顶点 **m** 的每个邻居顶点 **v**（且 **v** 不在生成树中），检查通过 **m** 到达 **v** 的路径是否更短。如果更短，就更新 **v** 的距离 **d[v]** 和父节点 **p[v]**。
- **第21行**：重复执行直到所有顶点都被访问，即最小生成树包含所有顶点。

### 3. **算法的特性**:

- **贪心策略**：每次选择与当前生成树相连的权重最小的边。
- **时间复杂度**：主要取决于 **优先队列** 的操作。若使用堆实现的优先队列，则构建队列的时间复杂度为 **O(m log n)**，每次删除最小元素的时间复杂度为 **O(log n)**，总共有 **n** 次删除操作，因此整体时间复杂度为 **O((n + m) log n)**。

### 4. **图示说明**:

- 图中展示了一个示例图，其中每条边的权重标记在边上。起始顶点 **A** 的最短路径初始化为 0，其他顶点的最短路径为正无穷。
- 算法会从起点 **A** 开始，逐步选择与已访问顶点相连的最小边并扩展生成树，直到所有顶点都被包含在内。

### 总结:

Prim 算法的核心思想是从一个起点开始，每次选择最短的边扩展生成树，直到所有顶点都被包含在最小生成树中。它适用于稠密图，时间复杂度依赖于优先队列的实现。

![image-20250411172803903](./READEME7.assets/image-20250411172803903.png)

这张图展示了 **Prim 算法** 的一个具体实现，下面是对其详细的解释：

### 1. **Prim 算法概述**：

- **输入**：一个图 **G** 和一个起始顶点 **s**。
- **输出**：一个最小生成树 **T**，它包含了图 **G** 中的所有顶点，且总权重最小。
...
- **第5-9行**：
  - 对于每个顶点 **v**，初始化顶点 **v** 的距离 **d[v]** 为正无穷（表示起始时无法到达其他顶点），并且将其父节点 **p[v]** 设置为 `NULL`。
  - 起始顶点 **s** 的距离 **d[s]** 设置为 0（表示与起始顶点的距离为 0）。

#### 优先队列：

- **第10-13行**：
  - 创建一个 **优先队列** **Q**，根据每个顶点的最短距离 **d[v]** 排序。
  - 使用 **Q.buildHeap()** 来构建一个最小堆，从图 **G** 的所有顶点中将其加入堆中。

#### 主要循环：

- **第14-22行**：
  - 循环次数为 **n**（图中顶点的数量）。
  - **第15行**：每次从队列中取出距离最小的顶点 **m**。
  - **第16行**：将顶点 **m** 添加到最小生成树 **T** 中。
  - **第17-22行**：对 **m** 的每个邻居顶点 **v**（且 **v** 不在最小生成树 **T** 中），检查通过 **m** 到达 **v** 的距离是否比现有的 **v** 的最短路径更短。如果更短，就更新 **v** 的距离，并将 **m** 设置为 **v** 的父节点。

#### 终止条件：

- 直到所有顶点都被访问并且生成树完成，即 **T** 中包含了所有顶点。

### 3. **图示说明**：

图中展示了一个包含 6 个顶点和 9 条边的图。每个顶点的邻接边权重标注在边上。通过 Prim 算法，我们从一个起始顶点（比如 **A**）开始，逐步扩展最小生成树，每次选择与当前生成树相连的最小边。

### 4. **表格说明**：

- 表格显示了每个顶点及其当前最短距离 **d[v]** 和父节点 **p[v]**。
  - 初始时，所有顶点的距离为无穷大（`inf`），除了起始顶点 **A**，它的距离为 0。
  - 在每一步算法中，顶点的最短路径（**d[v]**）和父节点（**p[v]**）会被更新。

### 5. **算法的核心思想**：

Prim 算法的核心思想是从一个起始顶点出发，逐步选择最短的边将未访问的顶点连接到生成树中，直到所有顶点都包含在最小生成树中。它利用了贪心算法的思想，每次选择当前权重最小的边。

### 6. **时间复杂度**：

- **构建优先队列**：构建堆的时间复杂度为 **O(m log n)**，其中 **m** 是边的数量，**n** 是顶点的数量。
- **每次移除最小元素**：每次移除最小元素的时间复杂度为 **O(log n)**，总共有 **n** 次移除操作。
- 整体时间复杂度为 **O((n + m) log n)**。

### 总结：

Prim 算法是一种用于计算最小生成树的贪心算法，它通过逐步扩展生成树，每次选择当前边权最小的边。该算法适用于稠密图，时间复杂度与优先队列的操作密切相关。

![image-20250411172907225](./READEME7.assets/image-20250411172907225.png)

这张图展示了 **Prim 算法** 的时间复杂度分析。我们可以从以下几个方面来理解其时间复杂度：

### 1. **Prim 算法概述**：

Prim 算法是一种基于贪心策略的最小生成树（MST）算法。它从一个顶点出发，通过逐步选择权重最小的边将图中的其他顶点连接到生成树中。

### 2. **算法伪代码分析**：

- **初始化阶段**：
  - 第7-9行：初始化每个顶点的距离 `d[v]` 为正无穷 (`+inf`)，并将父节点 `p[v]` 设置为 `NULL`，起始顶点 `s` 的距离 `d[s]` 设置为 0。这部分操作的时间复杂度是 **O(n)**，其中 **n** 是顶点的数量。
  - 第12-13行：使用优先队列（最小堆）来存储顶点，按照每个顶点的最短距离来进行排序。构建堆的时间复杂度是 **O(n log n)**。
- **主循环**：
  - 第16-22行：主循环运行 **n** 次（因为需要访问所有顶点）。
    - 第17行：每次从堆中移除一个最小元素。这个操作的时间复杂度是 **O(log n)**。
    - 第18-22行：对于当前顶点 **m**，检查它的所有邻居（尚未加入最小生成树的顶点），并更新其最短距离。如果找到更短的路径，就更新邻居的距离和父节点。这部分的时间复杂度是 **O(m)**，其中 **m** 是图中边的数量，因为每个邻居都需要访问一次。

### 3. **时间复杂度总结**：

- **初始化阶段**：构建堆的时间复杂度是 **O(n log n)**。
- **主循环**：在每次循环中，移除最小元素的操作需要 **O(log n)**，而更新邻居的距离需要 **O(m)**。

因此，Prim 算法的总体时间复杂度是：

$O((n + m) \log n)$

其中，**n** 是图中的顶点数，**m** 是图中的边数。

### 4. **优先队列的影响**：

在图的每一次更新过程中，我们需要使用 **优先队列** 来有效地找到当前的最小距离顶点。使用 **最小堆** 可以确保每次移除最小元素的操作是高效的，时间复杂度是 **O(log n)**。

### 5. **总结**：

- **时间复杂度**：`O((n + m) log n)`，其中 **n** 是顶点数，**m** 是边数。
- 使用 **优先队列**（最小堆）能有效地进行最小边的选择，确保算法的高效性。

![image-20250411173013265](./READEME7.assets/image-20250411173013265.png)

这张图展示了 **Prim 算法** 的代码、图示和时间复杂度分析。我们可以从以下几个方面来理解：

### 1. **Prim 算法概述**：

Prim 算法是一种基于贪心策略的最小生成树（MST）算法，旨在从一个起点出发，逐步扩展生成树，每次选择权重最小的边来加入生成树。

### 2. **图示与初始化**：

- **图示**：图中的顶点表示图中的节点，边表示节点之间的连接。
- **邻接矩阵**：通过邻接矩阵表示图，矩阵中的元素表示顶点之间的边的权重。如果没有边连接，则用 `∞` 表示。
- **状态表示**：每次迭代中，算法会选择一个当前距离最小的顶点，并更新它与相邻顶点之间的距离（`d[v]`）。

### 3. **算法的伪代码分析**：

- **初始化**：
  - 每个顶点的距离（`d[v]`）初始化为正无穷，表示初始时没有与任何顶点连接，起始顶点的距离设为 0。
  - 使用一个优先队列（最小堆）存储顶点，每个顶点按照与当前生成树的最小距离进行排序。
- **主循环**：
  - **第16-23行**：执行 **n** 次，每次选择一个顶点，移除当前距离最小的顶点，并将其加入到最小生成树中。
  - **更新邻居**：对于当前选择的顶点，检查与它相邻且未加入生成树的顶点，更新它们的最短距离。

### 4. **时间复杂度分析**：

- **邻接矩阵（Adj. Matrix）**：

  - **构建堆**：构建优先队列（堆）的时间复杂度是 **O(n log n)**。
  - **移除最小元素（`removeMin`）**：每次移除最小元素的操作需要 **O(log n)**。
  - **更新邻居**：在每次循环中，检查并更新所有与当前顶点相连的边，时间复杂度是 **O(n^2)**，因为对于每个顶点，我们可能需要遍历所有其他顶点。

  因此，使用邻接矩阵时，Prim 算法的总时间复杂度是 **O(n^2)**。

- **邻接表（Adj. List）**：

  - **构建堆**：构建优先队列（堆）的时间复杂度是 **O(n log n)**。
  - **移除最小元素**：每次移除最小元素的操作需要 **O(log n)**。
  - **更新邻居**：在每次循环中，更新邻居的时间复杂度为 **O(m)**，其中 **m** 是图中的边数，因为每个顶点仅需要访问其相邻的边。

  使用邻接表时，Prim 算法的总时间复杂度是 **O((n + m) log n)**。

### 5. **总结**：

- **邻接矩阵**：时间复杂度是 **O(n^2)**，适合边数较少的图。
- **邻接表**：时间复杂度是 **O((n + m) log n)**，适合稀疏图。

优先队列（最小堆）在 Prim 算法中的作用是非常重要的，它帮助我们高效地选择当前最小的边。

![image-20250411173343185](./READEME7.assets/image-20250411173343185.png)

这张图展示了 **Prim 算法** 和不同数据结构（邻接矩阵和邻接表）下的时间复杂度分析。

### **图解分析**：

- **Prim 算法的伪代码**：
  - 输入图 G 和一个起始顶点 s。
  - 初始化每个顶点的距离为正无穷，并将起始顶点的距离设置为 0。
  - 使用最小堆（或其他优先队列实现）来选择当前未加入生成树的顶点，按照与已选顶点的最小距离排序。
  - 每次循环，移除最小堆中的最小元素（即与生成树距离最小的顶点），并更新其相邻的未加入生成树的顶点的距离。
- **两种数据结构的时间复杂度**：
  - **邻接矩阵（Adj. Matrix）**：
    - **构建堆（Priority Queue）**：构建一个大小为 **n** 的堆需要 **O(n log n)** 时间。
    - **移除最小元素（removeMin）**：每次移除最小元素的操作是 **O(log n)**。
    - **更新邻居**：对于每个顶点，检查所有 **n** 个邻接点的时间复杂度为 **O(n²)**（因为需要检查矩阵中每一对元素）。
    - 因此，**邻接矩阵的总时间复杂度**为 **O(n² + m log n)**，其中 **n** 是顶点数，**m** 是边数。
  - **邻接表（Adj. List）**：
    - **构建堆（Priority Queue）**：构建堆的时间复杂度仍然是 **O(n log n)**。
    - **移除最小元素（removeMin）**：每次移除最小元素的操作是 **O(log n)**。
    - **更新邻居（update neighbors）**：
      - 对于每一个从堆中移除的顶点 m，我们需要检查它的所有邻居，以更新它们的最短路径。这里的每个邻居的更新都需要 O(log n) 的时间，因为每次更新一个邻居的距离时，需要在堆中进行调整。
      - 但是需要注意的是，我们并不是检查所有的 n 个顶点，而是仅仅检查与当前顶点 m 相邻的顶点，即与 m 相连的边。对于每个顶点，它的邻居数量是有限的，因此需要遍历的边数为图中所有的边数 m。因此，更新邻居的时间复杂度是 O(m log n)，这里的 m 表示图中的边数。
    - 因此，**邻接表的总时间复杂度**为 **O(n log n + m log n)**。

### **总结**：

- **邻接矩阵**：当图是密集的（即 **m** 接近 **n²**），使用邻接矩阵时的时间复杂度为 **O(n²)**，主要来源于更新邻居的部分。
- **邻接表**：当图是稀疏的（即 **m** 远小于 **n²**），使用邻接表时的时间复杂度为 **O(n log n + m log n)**，比邻接矩阵更加高效，尤其是在处理稀疏图时。

总体来说，邻接表适用于稀疏图，而邻接矩阵适用于密集图。在处理大规模的稀疏图时，通常更倾向于使用邻接表。

![image-20250411173757145](./READEME7.assets/image-20250411173757145.png)

在图的稠密度和稀疏度上，`Prim's Algorithm` 的时间复杂度差异在于图的结构：

- **稀疏图（Sparse Graph）**：稀疏图的边数 m 比较少，因此邻接表（Adj. List）的实现非常适用，时间复杂度为 **O(n log n + m log n)**，其中 **n** 是顶点数，**m** 是边数。
- **稠密图（Dense Graph）**：稠密图的边数 m 接近于顶点数的平方，因此邻接矩阵（Adj. Matrix）成为了一个更好的选择。邻接矩阵的实现时间复杂度为 **O(n²)**，因为我们需要检查所有顶点之间的边。

在图的实现中使用**未排序数组（Unsorted Array）**作为优先队列的实现时：

1. **构建优先队列**：对于每个顶点，我们需要将它放入一个未排序的数组中。每次插入一个元素的时间复杂度为 **O(1)**，因此构建队列的复杂度为 **O(n)**。
2. **移除最小元素**：在没有排序的数组中，找到最小元素需要遍历整个数组，因此每次 `removeMin` 操作的时间复杂度是 **O(n)**。

对于**稠密图**，在这种情况下使用未排序的数组时，总体的时间复杂度为 **O(n²)**，因为在每个 `removeMin` 操作中，我们需要检查 **n** 次，而此操作会进行 **n** 次。

### 总结：

- **Heap**：优先队列使用堆，操作是对数复杂度（`O(log n)`），因此时间复杂度更适合图的稀疏结构和大规模问题。
- **Unsorted Array**：未排序数组的优点是实现简单，但由于每次 `removeMin` 操作需要 **O(n)** 的时间，所以在稠密图上会变得非常低效。

![image-20250411173923193](./READEME7.assets/image-20250411173923193.png)

这张图片展示了最小生成树（MST）算法的运行时间，特别是 **Kruskal 算法** 和 **Prim 算法**。
...
- **时间复杂度：** $O(n + m \log n)$
  ...
  - **m** 是图中的边数。
- 这个时间复杂度的含义是：
  - 需要 **O(n)** 的时间来初始化 **n** 个顶点。
  - 需要 **O(m \log n)** 的时间来处理 **m** 条边。每次处理一条边时，边的排序操作（使用并查集）会有对数时间复杂度。
...
- **时间复杂度：** $O(n \log n + m \log n)$
  ...
  - **m** 是图中的边数。
- 这个时间复杂度的含义是：
  - **O(n \log n)**：由于使用优先队列（堆）来处理顶点，插入和删除最小元素的操作每个的时间复杂度是 **O(\log n)**，而总共有 **n** 个顶点，所以是 **O(n \log n)**。
  - **O(m \log n)**：每条边的处理涉及到对每个顶点的邻接点进行更新，涉及到对优先队列的操作，时间复杂度是 **O(\log n)**，因此总共的时间复杂度为 **O(m \log n)**。

### **Sparse Graph (稀疏图)** 和 **Dense Graph (稠密图)**:

- **稀疏图（Sparse Graph）** 指的是边数 **m** 相对较少的图，适合用 Kruskal 算法。Kruskal 算法的时间复杂度是 **O(n + m \log n)**，其优势在于稀疏图的边数较少，排序操作的复杂度较低。
- **稠密图（Dense Graph）** 指的是边数 **m** 接近于 **n²** 的图，适合用 Prim 算法。Prim 算法的时间复杂度是 **O(n \log n + m \log n)**，在稠密图中，由于边数较多，处理每条边的时间复杂度影响较大。

因此，根据图的稀疏度，选择不同的算法可能会对算法的性能产生显著影响。

![image-20250411174151336](./READEME7.assets/image-20250411174151336.png)这张图片展示了 **Prim's Algorithm** 的时间复杂度更新，考虑了使用不同类型的堆（**Binary Heap** 和 **Fibonacci Heap**）。

### 1. **二叉堆（Binary Heap）**

- **Remove Min** 操作的时间复杂度：$O(\log n)$
  - 二叉堆的 **removeMin** 操作需要对堆进行调整，时间复杂度是 $O(\log n)$。
- **Decrease Key** 操作的时间复杂度：$O(\log n)$
  - 对堆中的元素进行 **Decrease Key** 操作时，需要重新调整堆，时间复杂度也是 $O(\log n)$。

### 2. **斐波那契堆（Fibonacci Heap）**

- **Remove Min** 操作的时间复杂度：$O(\log n)$
  - 斐波那契堆的 **removeMin** 操作的时间复杂度是 $O(\log n)$，但常数因素会比二叉堆低。
- **Decrease Key** 操作的时间复杂度：$O(1)$
  - 斐波那契堆的 **Decrease Key** 操作非常高效，时间复杂度是 $O(1)$，这使得它在某些场景下比二叉堆更加高效。

### 影响：

- 使用 **二叉堆（Binary Heap）**，在 **Prim's Algorithm** 中的时间复杂度为 $O(n \log n + m \log n)$，因为每次 **removeMin** 和 **Decrease Key** 操作的时间复杂度是 $O(\log n)$。
- 使用 **斐波那契堆（Fibonacci Heap）**，时间复杂度也为 $O(n \log n + m \log n)$，但是 **Decrease Key** 操作的时间复杂度为 $O(1)$，这使得斐波那契堆在边的更新操作非常高效，从而优化了算法的性能，尤其在处理稠密图时具有优势。

### 总结：

- 在 **稠密图（Dense Graph）** 中，斐波那契堆可能会比二叉堆更有效，特别是在需要频繁进行 **Decrease Key** 操作时。
- 然而，**二叉堆（Binary Heap）** 在实现和实际操作中通常会更加简单和高效，适用于大多数情况。