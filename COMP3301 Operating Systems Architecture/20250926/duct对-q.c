...
#define A2_EV_RXDROP      (1U << 2)  /* command completion event */
#define A2_EV_CMDCOMP     (1U << 3)  /* RX drop event */
...
int duct_tx_submit(struct duct_softc *, const void *, size_t, uint32_t);
...
ductopen(dev_t dev, int mode, int flags, struct proc *p)
{
    ...
    /* Allocate RX queue if not already allocated */
    if (sc->sc_rx_queue == NULL) {
        sc->sc_rx_qsize = 64;
        sc->sc_rx_queue = mallocarray(sc->sc_rx_qsize,
            sizeof(struct duct_packet_hdr), M_DEVBUF, M_NOWAIT | M_ZERO);
        sc->sc_rx_payload = mallocarray(sc->sc_rx_qsize,
            sizeof(void *), M_DEVBUF, M_NOWAIT | M_ZERO);
        sc->sc_rx_payload_len = mallocarray(sc->sc_rx_qsize,
            sizeof(u_int), M_DEVBUF, M_NOWAIT | M_ZERO);
        if (sc->sc_rx_queue == NULL || sc->sc_rx_payload == NULL ||
            sc->sc_rx_payload_len == NULL) {
            printf("%s: failed to allocate RX queues\n", sc->sc_dev.dv_xname);
            goto fail_rx;
        }
        sc->sc_rx_qhead = sc->sc_rx_qtail = 0;
    }

    /* Allocate TX queue if not already allocated */
    if (sc->sc_tx_queue == NULL) {
        sc->sc_tx_queue = mallocarray(sc->sc_tx_size,
            sizeof(void *), M_DEVBUF, M_NOWAIT | M_ZERO);
        if (sc->sc_tx_queue == NULL) {
            printf("%s: failed to allocate TX queue\n", sc->sc_dev.dv_xname);
            goto fail_tx;
        }
    }
...
fail_tx:
    if (sc->sc_rx_queue) {
        free(sc->sc_rx_queue, M_DEVBUF,
             sc->sc_rx_qsize * sizeof(struct duct_packet_hdr));
        sc->sc_rx_queue = NULL;
    }
    if (sc->sc_rx_payload) {
        free(sc->sc_rx_payload, M_DEVBUF,
             sc->sc_rx_qsize * sizeof(void *));
        sc->sc_rx_payload = NULL;
    }
    if (sc->sc_rx_payload_len) {
        free(sc->sc_rx_payload_len, M_DEVBUF,
             sc->sc_rx_qsize * sizeof(u_int));
        sc->sc_rx_payload_len = NULL;
    }
fail_rx:
    sc->sc_open = 0;
...
ductclose(dev_t dev, int flags, int mode, struct proc *p)
{
    ...
    /* Free TX queue */
    if (sc->sc_tx_queue) {
        free(sc->sc_tx_queue, M_DEVBUF,
             sc->sc_tx_size * sizeof(void *));
        sc->sc_tx_queue = NULL;
    }
...
ductwrite(dev_t dev, struct uio *uio, int ioflag)
{
    ...
    kbuf = malloc(total_len, M_DEVBUF, M_NOWAIT);
    if (kbuf == NULL)
        return (ENOMEM);
    ...
    /* Default destination: echo if not set */
    uint32_t dst = (hdr.dpkt_destination != 0) ? hdr.dpkt_destination : 0xEC60;
...
retry:
    ...
    /* Safe to submit now */
    error = duct_tx_submit(sc, kbuf, total_len, dst);
...
duct_intr_event(void *arg)
{
    ...
    if (evflags & A2_EV_TXCOMP) {
        ...
        printf("%s: [bit0:TXCOMP] TX completion event\n", sc->sc_dev.dv_xname);
        printf("%s: TX ring debug: tx_ring_size=%zu sc_tx_size=%u desc_size=%zu\n",
            sc->sc_dev.dv_xname,
            tx_ring_size,
            sc->sc_tx_size,
            sizeof(struct a2_tx_desc));
    ...
    if (evflags & A2_EV_RXDROP) {
        printf("%s: RX drop detected\n", sc->sc_dev.dv_xname);
    }

    /* Command completions */
    if (evflags & A2_EV_CMDCOMP) {
        printf("%s: Command completion event\n", sc->sc_dev.dv_xname);
...
duct_rx_process(struct duct_softc *sc)
{
    ...
    while (sc->sc_rx_cons != sc->sc_rx_prod) {
        ...
        /* Debug: show descriptor info */
        printf("%s: RX idx=%u owner=%02x pktlen=%u len0=%u src=0x%08x dst=0x%08x\n",
            sc->sc_dev.dv_xname, idx,
            desc->a2rd_owner, desc->a2rd_pktlen, desc->a2rd_length[0],
            desc->a2rd_src, desc->a2rd_dst);

        /* Sync RX buffer so CPU sees device-written data */
        bus_dmamap_sync(sc->sc_dmat, sc->sc_rx_buf_maps[idx], 0,
            MIN(desc->a2rd_pktlen, sc->sc_rx_buf_size),
            BUS_DMASYNC_POSTREAD);

        pkt_hdr = (struct duct_packet_hdr *)sc->sc_rx_buf_kva[idx];

        /* Overwrite header src/dst with hardware values */
        pkt_hdr->dpkt_source      = desc->a2rd_src;
        pkt_hdr->dpkt_destination = desc->a2rd_dst;

        /* Compute actual payload length from descriptor */
        size_t payload_len = 0;
        if (desc->a2rd_pktlen > sizeof(struct duct_packet_hdr)) {
            payload_len = desc->a2rd_pktlen - sizeof(struct duct_packet_hdr);
        }
        pkt_hdr->dpkt_length = payload_len;   /* ensure header length matches actual */

        /* Prepare payload copy */
        void *payload_buf = NULL;
        if (payload_len > 0) {
            payload_buf = malloc(payload_len, M_DEVBUF, M_NOWAIT);
            if (payload_buf != NULL) {
                memcpy(payload_buf,
        ...
        if (next_tail != sc->sc_rx_qhead) {
            ...
            sc->sc_rx_payload[sc->sc_rx_qtail] = payload_buf;
            sc->sc_rx_payload_len[sc->sc_rx_qtail] = payload_len;
            sc->sc_rx_qtail = next_tail;

            printf("%s: RX queued idx=%u src=0x%08x dst=0x%08x len=%zu\n",
                sc->sc_dev.dv_xname, idx,
                pkt_hdr->dpkt_source, pkt_hdr->dpkt_destination,
                payload_len);
            ...
            /* Queue full, drop */
            if (payload_buf != NULL)
                free(payload_buf, M_DEVBUF, payload_len);
        }
        mtx_leave(&sc->sc_mtx);
 ...
 */
int
duct_tx_submit(struct duct_softc *sc, const void *data, size_t len, uint32_t dst)
{
    struct a2_tx_desc *desc;
    const struct duct_packet_hdr *uhdr = (const struct duct_packet_hdr *)data;
    u_int next_prod;

    /* Check if TX ring is full */
    next_prod = (sc->sc_tx_prod + 1) % sc->sc_tx_size;
    if (next_prod == sc->sc_tx_cons) {
        printf("%s: TX ring full\n", sc->sc_dev.dv_xname);
        return (ENOBUFS);
    }

    /* Check packet size */
    if (len > sc->sc_tx_buf_size) {
        printf("%s: packet too large: %zu > %u\n",
            sc->sc_dev.dv_xname, len, sc->sc_tx_buf_size);
        return (EMSGSIZE);
    }
    ...
    /* Copy data (header + payload) to TX buffer */
    ...
    /*
     * Correct descriptor fields:
     * - pktlen = sizeof(header) + payload length
     * - length[0] = payload length only
     */
    desc->a2td_pktlen     = sizeof(*uhdr) + uhdr->dpkt_length;
    desc->a2td_length[0]  = uhdr->dpkt_length;
    desc->a2td_ptr[0]     = sc->sc_tx_buf_pa[sc->sc_tx_prod];
    desc->a2td_src        = sc->sc_hwaddr;  /* local hwaddr */
    desc->a2td_dst        = dst;            /* destination addr */

    /* Debug */
    printf("%s: TX debug pktlen=%u length0=%u hdr.dpkt_length=%u dst=0x%08x\n",
        sc->sc_dev.dv_xname,
        desc->a2td_pktlen, desc->a2td_length[0],
        uhdr->dpkt_length, dst);

    /* Give ownership to device */
    membar_producer();
    desc->a2td_owner = A2_OWNER_DEVICE;

    /* Sync descriptor to device */
    ...
    /* Ring doorbell with high bit set */
    ...
    bus_space_barrier(sc->sc_iot, sc->sc_ioh, A2_REG_DBELL,
        4, BUS_SPACE_BARRIER_WRITE);

    printf("%s: TX packet submitted: len=%zu, prod=%u, dst=0x%08x\n",
        sc->sc_dev.dv_xname, len, sc->sc_tx_prod, dst);
    return 0;
}
