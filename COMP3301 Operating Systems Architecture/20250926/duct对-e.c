...
duct_rx_process(struct duct_softc *sc)
{
    ...
    while (sc->sc_rx_cons != sc->sc_rx_prod) {
        ...
        if (desc->a2rd_pktlen < sizeof(struct duct_packet_hdr) ||
            (pkt_hdr->dpkt_length > 0 &&
             desc->a2rd_pktlen < sizeof(struct duct_packet_hdr) + pkt_hdr->dpkt_length)) {
            goto recycle;
        }

        /* Prepare payload copy outside lock */
        void *payload_buf = NULL;
        if (pkt_hdr->dpkt_length > 0) {
            payload_buf = malloc(pkt_hdr->dpkt_length, M_DEVBUF, M_NOWAIT);
            if (payload_buf != NULL) {
                memcpy(payload_buf,
        ...
        if (next_tail != sc->sc_rx_qhead) {
            ...
            sc->sc_rx_payload_len[sc->sc_rx_qtail] =
                (payload_buf != NULL) ? pkt_hdr->dpkt_length : 0;
            ...
            /* Queue full, drop */
            if (payload_buf != NULL)
                free(payload_buf, M_DEVBUF, pkt_hdr->dpkt_length);
        }
        mtx_leave(&sc->sc_mtx);
...
duct_tx_submit(struct duct_softc *sc, const void *data, size_t len, uint32_t dst)
{
    ...
    desc->a2td_src        = sc->sc_hwaddr;  /* local hardware address */
    desc->a2td_dst        = dst;            /* destination address */
